(function($) {
    ...
    var clickId,
        ...
        click_go = function(a, options) {
            comebacker_launch = false;
            var hash = $.url(location.href).param('hash');
            if (landUrl.length > 0) document.location = landUrl + (hash ? `&hash=${hash}` : '');
            else $.ajax({
                url: options.landUrl,
    ...
    $.fn.getUrlHash = function () {
        return new URLSearchParams(document.location.href).get("hash")
    };
    $.fn.leadprofit = function(options) {
        //save land hash to cookies
        var hash = $.fn.getUrlHash() || options.hash
        $.cookie("landingHash", hash, { expires: 30, path: '/' });
        // save all the query params to cookie if they are exists
        $.fn.leadprofitSaveQueryString(options);
        // end new functionality for init
        ...
        options = $.extend({
            ...
            successCallback: function(form, idOrder) {
                ...
                function successHandler () {
                    options.openNewTab ? openInNewTab() : changeLocation()
                }
    ...
    $.fn.leadprofitConfirm = function(options) {
        ...
        // подписываемся на переотправку лидов со страницы подверждения
        $.fn.resendLeadFromConfirmPage();
        return this;
    };
    ...
    $.fn.leadprofitSaveQueryString = function (optionsObj) {
        var queryString = '';
        // если опций не передали, то проверим, есть ли у нас что-то в урл параметрах и складываем их в строку, которую позже запомним в куках
        ...
        // проверяем, передали ли нам опции и есть ли там что-то, кроме хэша + кладем в конец строки все то, что было на ленде как параметры
        if (optionsObj && Object.keys(optionsObj).length > 1) {
            // берем все, кроме хэша
            var params = Object.entries(optionsObj).filter(function(el){if (el[0] !== 'hash') return el})
            if (queryString.length) queryString += "&"
            params.forEach(function (param, index) {
                queryString += param[0] + '=' + param[1]
                if (index !== params.length-1) queryString += "&"
            })
        }
        // если что-то есть, то кладем в куки
        if (queryString.length) $.cookie("query", queryString, { expires: 30, path: '/' });
    };
    // resend leads from confirm page, instead of 
    $.fn.resendLeadFromConfirmPage = function() {
        var resendPopup = document.querySelector('#resend-lead-popup')
        // exit to default behavior if no hash for creating new lead or button for creating new lead
        if (!resendPopup || !$('#leadprofit-returnurl') || !$.cookie("landingHash") || $.alredyResend) return;;
        $.fn.subscribeConfirmResendLead(resendPopup)
        
    };
    $.fn.subscribeConfirmResendLead = function (popup) {
        var activePopupClass = 'resend-lead-popup--active'
        var showPopupTriggers = [
            document.querySelector('#leadprofit-returnurl'),
            popup.querySelector('.resend-lead-popup__close')
        ]
        // подписывемся на триггеры показа и на всякий перевентим событие переадресации для линки + убираем/добавляем активный класс попапу
        showPopupTriggers.forEach(function (trigger) {
            trigger.addEventListener('click', function(event) {
                event.preventDefault()
                // и закроем все уведомления
                if (popup.classList.contains(activePopupClass)) {
                    popup.classList.remove(activePopupClass)
                    var notifs = popup.querySelectorAll('.notification')
                    notifs.forEach(function(n) {n.classList.remove('active')})
                    return;
                }
                // если мы попап выше не закрыли - то, очевидно, откроем его сейчас)
                popup.classList.add(activePopupClass)
            })
        })
        var resendForm = popup.querySelector('form');
        // подставляем в форму данные, которые пользователь отправил ранее
        resendForm.querySelector('input[name=name]').value = $.cookie("name")
        resendForm.querySelector('input[name=phone]').value =$.cookie("phone")
        // подписываемся на сабмит этой формы
        resendForm.addEventListener('submit', function(event) {
            // сразу превентим перезагрузку
            event.preventDefault()
            // добавляем новое поле, которое отвечает за состояние запросов и ресенда
            if (!$.confirmResendUtilityData) {
                $.confirmResendUtilityData = {}
            }
            // если один запрос уже отправлен, то ничего не делаем далее
            if ($.confirmResendUtilityData.isOnProgress) return;
            if ($.confirmResendUtilityData.error && $.cookie("return")) window.location.href = $.cookie("return")
            // тут вызываем метод, который отвечает за отправку лида
            $.fn.resendLeadConfirmSubmit(resendForm)
        })
    };
    $.fn.resendLeadConfirmSubmit = function (form) {
        // сразу определим, что запрос отправлен
        $.confirmResendUtilityData.isOnProgress = true
        // создадим исходные данные
        var leadData = {
            name: form.querySelector('input[name=name]').value,
            phone: form.querySelector('input[name=phone]').value
        }
        // проверим, есть ли параметры, которые были при генерации прошлого клика на ленде
        var clickUrl = "//click.lucky.online/click/click.html"
        var params = $.cookie("query")
        // и если параметры есть, ты просто добавим их к урлу ленда
        if (params && params.length) clickUrl = clickUrl + "?" + params
        // отправим запрос на генерацию click_id
        $.ajax({
            url: clickUrl,
            data: { hash: $.cookie("landingHash") },
            dataType: 'jsonp',
            jsonpCallback: 'callback',
            type: "GET",
            success: function(data) {
                $.confirmResendUtilityData.clickId = data.clickId
                $.confirmResendUtilityData.country = data.country
                // добавим эти данные в  данные формы
                leadData.click_id = data.clickId
                leadData.country = data.country
                // если оба есть - отправим запрос на создание лида
                $.fn.confirmPageResendLead(leadData)
                
            },
            error: function (error) {
                $.confirmResendUtilityData.error = true
                document.querySelector('#resend-lead-popup .notification.error').classList.add('active')
            }
        });
    };
    $.fn.confirmPageResendLead = function (payload) {
        $.ajax({
            url: "//click.lucky.online/lead/direct.html",
            data: payload,
            dataType: 'jsonp',
            jsonpCallback: 'callback',
            type: "GET",
            success: function(data) {
                $.confirmResendUtilityData.isOnProgress = false
                $.cookie("name", payload.name, { expires: 30, path: '/'  });
                $.cookie("phone", payload.phone, { expires: 30, path: '/'  });
                document.querySelector('#resend-lead-popup .notification.success').classList.add('active')
                
                // Обновляем контент на самом ленде, чтобы не смутить пользователя
                $('#leadprofit-name').text($.cookie("name"));
                $('#leadprofit-phone').text($.cookie("phone"));
            }
        });
